...
from metrics import iou_score, dice_coef, recall_s, precision_s, accuracy_s
...
def parse_args():
    ...
    # model
    parser.add_argument('--arch', '-a', metavar='ARCH', default='BiFormer_resnet18',
                        choices=ARCH_NAMES,
...
def train(config, train_loader, model, criterion, optimizer):
    ...
    for input, target, _ in train_loader:
        ...
        if config['deep_supervision']:
            ...
            output = model(input)
            if isinstance(output, tuple):  # 检查是否为元组输出
                output = output[-1]  # 取最后一个特征图作为主输出
...
def main():
    ...
    torch.backends.cudnn.benchmark = False  # 添加这一行以确保确定性
    os.environ['PYTHONHASHSEED'] = str(seed)  # 添加这一行以确保Python哈希的一致性
    # ===== 设置随机种子 =====
    ...
    # Data loading code
    img_idst = glob(os.path.join('arcade', "train", 'images', '*' + config['img_ext']))
    img_idst = [os.path.splitext(os.path.basename(p))[0] for p in img_idst]

    # Data loading code
    img_idsv = glob(os.path.join('arcade', "val", 'images', '*' + config['img_ext']))
    ...
    train_dataset = Dataset(
        ...
        img_dir=os.path.join('arcade', "train", 'images'),
        mask_dir=os.path.join('arcade', "train", 'masks'),
    ...
    val_dataset = Dataset(
        img_ids=val_img_ids,
        img_dir=os.path.join('arcade', "val", 'images'),
        mask_dir=os.path.join('arcade', "val", 'masks'),
