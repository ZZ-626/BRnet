...
def is_list_of(seq, cls_or_tuple):
    """Check whether the elements in sequence are all instances of some type.

    Args:
        seq (Sequence): The sequence to be checked.
        cls_or_tuple (type or tuple[type]): Data type or data type tuple.

    Returns:
        bool: Whether the elements in sequence are all instances of the given type.
    """
    if not isinstance(seq, Sequence):
        return False
    for item in seq:
        if not isinstance(item, cls_or_tuple):
            return False
    return True
