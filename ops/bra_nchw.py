...
class nchwBRA(nn.Module):
    ...
    * different pooling behavior (count_include_pad=False)
    ...
    def __init__(self, dim, num_heads=8, n_win=7, qk_scale=None, topk=4,  side_dwconv=3, auto_pad=False, attn_backend='torch'):
        ...
        self.scale = qk_scale or self.dim ** -0.5 # NOTE: to be consistent with old models_cls.
    ...
    def forward(self, x:torch.Tensor, ret_attn_mask=False):
        """
        Args:
            x: NCHW torch.Tensor, better to be channel_last (https://pytorch.org/tutorials/intermediate/memory_format_tutorial.html)
        Return:
            NCHW torch.Tensor
        """
        N, C, H, W = x.size()
        ...
        _, idx_r = torch.topk(a_r, k=self.topk, dim=-1) # n(hw)k long torch.Tensor
        idx_r:torch.Longtorch.Tensor = idx_r.unsqueeze_(1).expand(-1, self.num_heads, -1, -1) 
