...
class TopkRouting(nn.Module):
    ...
    def forward(self, query:torch.Tensor, key:torch.Tensor)->Tuple[torch.Tensor]:
        """
        Args:
            q, k: (n, p^2, c) torch.Tensor
        Return:
            r_weight, topk_index: (n, p^2, topk) torch.Tensor
        """
...
class KVGather(nn.Module):
    ...
    def forward(self, r_idx:torch.Tensor, r_weight:torch.Tensor, kv:torch.Tensor):
        """
        r_idx: (n, p^2, topk) torch.Tensor
        r_weight: (n, p^2, topk) torch.Tensor
        kv: (n, p^2, w^2, c_kq+c_v)

        Return:
            (n, p^2, topk, w^2, c_kq+c_v) torch.Tensor
        """
...
class BiLevelRoutingAttention(nn.Module):
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        r_weight, r_idx = self.router(q_win, k_win) # both are (n, p^2, topk) torch.Tensors
        ...
        q_pix = rearrange(q_pix, 'n p2 w2 (m c) -> (n p2) m w2 c', m=self.num_heads) # to BMLC torch.Tensor (n*p^2, m, w^2, c_qk//m)
